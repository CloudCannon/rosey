...
const JsonTranslator = require('./plugins/json/JsonTranslator');
...
function cloneAssets(options) {
	...
	const filterFunc = (source) => !options.rosey.exclusions.test(source);
...
function translateHtmlPages(options) {
	...
	return async.each(localeNamesFiltered, async (targetLocale, next) => {
		...
		const htmlTranslator = new HTMLTranslator(translateOptions);
		const jsonTranslator = new JsonTranslator(translateOptions);
		await Promise.all([
			htmlTranslator.translateFiles(),
			jsonTranslator.translateFiles()
		]).then(next);
	});
}
...
function clonePrelocalisedHtmlPages(options) {
	log('Starting cloning prelocalised pages.');
	...
	return new Promise((resolve) => {
		vfs.src(`${options.rosey.source}/**/*.html`)
			.pipe(rosey.redirectPage({
				...
				redirect_page: options.rosey.redirect_page
...
module.exports = {
	...
	build: async function (options) {
		...
		const cloneAssetsProm = cloneAssets(options)
			.then(() => { log(c.green('Finished cloning assets.')); });
		...
		const clonePrelocalisedHtmlPagesProm = clonePrelocalisedHtmlPages(options)
			.then(() => { log(c.green('Finished cloning prelocalised pages.')); });
