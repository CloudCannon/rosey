...
module.exports = class HTMLTranslator extends HTMLParserInterface {
	...
	rewriteImagePaths(file, src) {
		...
		const translatedFileName = fileName.replace(extension, `.${this.targetLocale}${extension}`);
		const translatedSrc = src.replace(fileName, translatedFileName);

		const fileLocation = path.join(this.options.rosey.full_images_source, translatedSrc);

		if (fs.pathExistsSync(fileLocation)) {
			returnedSrc = path.join(this.options.rosey.images_source.replace(this.options.rosey.source, ''), translatedSrc);
			if (this.options.rosey.show_skipped_updates) {
				log(`Translating image ${c.green(`'${fileLocation}'`)}`);
			}
		} else if (this.options.rosey.show_skipped_updates) {
			log(`Skipping image ${c.grey(`'${fileLocation}'`)}`);
		}
