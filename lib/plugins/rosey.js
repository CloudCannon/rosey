...
function returnNamespace($el, options) {
	let namespace = $el.parents(`[${options.tag}-root]`).attr(`${options.tag}-root`);
	if (namespace !== '') { // Used to clean any other namespace previously
		if (namespace) {
			namespace += ':';
		} else {
			const parentNamespaces = $el.parents(`[${options.tag}-ns]`);
			if (parentNamespaces) {
				namespace = '';
				for (let i = parentNamespaces.length - 1; i >= 0; i -= 1) {
					const ns = parentNamespaces[i].attribs[`${options.tag}-ns`];
...
function handleHTMLFile(options) {
	...
	return through(function transform(file, encoding, callback) {
		...
		$(`[${options.tag}]`).each(function processElement() {
			...
			const namespace = returnNamespace($el, options);
		...
		$(`[${options.tag}-attrs-explicit]`).each(function processExplicitAttr() {
			...
			// const namespace = $el.parents(`[${options.tag}-ns]`).attr(`${options.tag}-ns`);
			const namespace = returnNamespace($el, options);
			const explicit = JSON.parse(attrStr);

			options.processExplicitAttr.apply(this, [file, $el, explicit, namespace, $]);
		});
