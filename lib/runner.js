...
const JsonGenerator = require('./plugins/json/Generator');
		...
		promises.push(new Promise((resolve) => {
			vfs.src(`${options.rosey.full_source}/**/*.html`)
				.pipe(rosey.generate({
					...
					tag: options.rosey.data_tag,
					generatedFileName: options.rosey.generated_locale_dest_file
				}))
				.pipe(vfs.dest(options.rosey.full_generated_locale_dest_path, {overwrite: false}))
				.on('end', resolve);
		}));
		promises.push(jsonGenerator.readFiles());
