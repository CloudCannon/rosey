...
/**
*  Check if targetLocaled is supposed to be translated or not
*
* @param {Object} options The options object.
* @param {String} targetLocale The options object.
* @returns {Boolean} true if targeLocale is to be skipped. False if not.
*/
function skipTransalateLocale(options, targetLocale) {
  // log(`Checking for translations from ${targetLocale}`);
  let skipTranslate = false;
  // Skip if targetLocale is no on the list of partialLanguages to be parsed.
  if (options.flags.partialLanguages
      && options.flags.partialLanguages.indexOf(targetLocale.toUpperCase()) < 0) {
    log(`${targetLocale} not on specified languages to be translated`);
    skipTranslate = true;
  }
...
function translateHtmlPages(options) {
  ...
  return async.each(localeNamesFiltered, (targetLocale, next) => {
    vfs.src(`${options.rosey.source}/**/*.html`)
      .pipe(rosey.translate({
        showSkippedUpdates: options.rosey.show_skipped_updates,
        showMissingLocaleWarnings: options.rosey.show_missing_locale_warnings,
      ...
      .pipe(vfs.dest(path.join(options.rosey.dest, targetLocale)))
      .on('end', next);
  });
}
...
function clonePrelocalisedHtmlPages(options) {
  ...
  return async.each(localeNamesFiltered, (targetLocale, next) => {
    vfs.src(`${options.rosey.source}/${targetLocale}/**/*.html`)
      .pipe(rosey.translate({
        showSkippedUpdates: options.rosey.show_skipped_updates,
        showMissingLocaleWarnings: options.rosey.show_missing_locale_warnings,
...
async function wrapCharacters(options) {
  ...
  await fs.ensureDir(wrappedDir).then(() => async.eachSeries(localeNames, (targetLocale, next) => {
    // Skip if targetLocale is not on the list of partialLanguages to be parsed.
    if (skipTransalateLocale(options, targetLocale)) {
      return next();
    }
...
module.exports = {
  ...
  generate(options) {
    ...
    return new Promise((resolve) => {
      vfs.src(`${options.rosey.full_source}/**/*.html`)
        .pipe(rosey.generate({
          version: options.rosey.source_version,
          delimeter: options.rosey.source_delimeter,
          showDuplicateLocaleWarnings: options.rosey.show_duplicate_locale_warnings,
          tag: options.rosey.data_tag,
        }))
        .pipe(vfs.dest(options.rosey.full_generated_locale_dest))
        .on('end', resolve);
    });
  },
  ...
  check(options) {
    return new Promise((resolve, reject) => {
      readLocalesFromDir(options.rosey.locale_source, (errReadLocales, returnedLocales) => {
        ...
        log(`Loading ${path.join(options.rosey.generated_locale_dest, 'source.json')}...`);
        fs.readFile(path.join(options.rosey.generated_locale_dest, 'source.json'), (errReadFile, data) => {
          if (errReadFile) {
            log(`${c.red('Unable to read source')} from ${
              c.blue(path.join(options.rosey.generated_locale_dest, 'source.json'))}: ${errReadFile.message}`);
          ...
          const outputFilename = path.join(options.rosey.generated_locale_dest, 'checks.json');
  ...
  convert(options) {
    return new Promise((resolve, reject) => {
      log(`Loading ${path.join(options.rosey.generated_locale_dest, 'source.json')}...`);
      fs.readJSON(path.join(options.rosey.generated_locale_dest, 'source.json'))
        .then((source) => {
          if (!source.version || source.version < 2) {
            const errVersion = new Error('Convert is only possible from a Version 2 source.json file. You must run `rosey generate --version 2` before using the `convert` command');
        ...
        .catch((errReadingSource) => {
          log(c.red('Unable to read source.json file'));
