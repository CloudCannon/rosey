  ...
  fs.readdir(dir, (err, files) => {
    ...
    async.each(files, (filename, next) => {
      ...
      fs.readFile(path.join(dir, filename), (err, data) => {
        ...
        for (const localeKey in returnedLocales[key]) {
...
async function wrapCharacters(options) {
  ...
  await fs.ensureDir(wrappedDir).then(() => async.eachSeries(localeNames, (targetLocale, next) => {
    ...
    fs.readFile(inputFilename, (err, data) => {
      ...
      wordwrapLocale(targetLocale, data.toString('utf8'), (err, output) => {
        if (err) {
...
function loadWordwraps(options) {
  return new Promise((resolve) => {
    ...
    readLocalesFromDir(wrappedDir, (err, returnedLocales) => {
      if (!err) {
        for (const localeName in returnedLocales) {
...
module.exports = {
  ...
  check(options) {
    return new Promise((resolve, reject) => {
      readLocalesFromDir(options.i18n.locale_source, (err, returnedLocales) => {
        if (err) {
          log(`${c.red('Unable to read locales')} from ${
        ...
        fs.readFile(path.join(options.i18n.generated_locale_dest, 'source.json'), (err, data) => {
          if (err) {
            // log(err);
            log(`${c.red('Unable to read source')} from ${
              c.blue(path.join(options.i18n.generated_locale_dest, 'source.json'))}: ${err.message}`);
          ...
          function compareTranslations(source, target) {
            ...
            if (options.i18n.source_version > 1) {
              const sourceString = source.original;
  ...
  async build(options) {
    ...
    const del = await this.clean(options);
